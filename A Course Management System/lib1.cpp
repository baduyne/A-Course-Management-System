...
{
	for (node_sv* i = l.head; i->next!= NULL; i = i->next)
	{
		for (node_sv* j = i->next; j != NULL; j = j->next)
		{
			if (strcmp(i->data.ma_sinh_vien, j->data.ma_sinh_vien) > 0)
				hoa_doi_sv(i->data, j->data);
		}
	}
}
...
void xuat_ds_sinh_vien_lop_hoc(ds_sinh_vien l, int x ,int y, int pos)
{
	x = 15;
	XoaManHinh();
	...
	while (l.head != NULL)
	{
		...
		gotoxy(x, y);
		ToMau(x-1, y,c, 7);
		ToMau(x, y, l.head->data.ma_sinh_vien, color);
		ToMau(x +10, y, c, 7);
		ToMau(x + 11, y, l.head->data.ho_ten, color);
		ToMau(x + 33, y, c, 7);
		ToMau(x + 34, y, l.head->data.gioi_tinh, color);
		ToMau(x + 45, y, c, 7);
		ToMau(x + 46, y, l.head->data.ngay_sinh, color);
		ToMau(x + 59, y, c, 7);
		ToMau(x + 60, y, l.head->data.cmnd, color);
		ToMau(x + 75, y, c, 7);
		l.head = l.head->next;
		y++;
		i++;
...
void ghi_ds_sinh_vien_vao_lop_hoc(nam_hoc a,lop_hoc c, ds_sinh_vien infor)
{
	...
	while (infor.head != NULL)
	{
		if (i == c.so_luong + 1)
			break;
		filesv << i << ',' << infor.head->data.ma_sinh_vien << ',' << infor.head->data.ho_ten << ',' << infor.head->data.gioi_tinh << ',' << infor.head->data.ngay_sinh << ',' << infor.head->data.cmnd << endl;
        infor.head = infor.head->next;
		i++;
	}
	filesv.close();
...
void ghi_ds_lop_hoc_vao_file(nam_hoc b, lop_hoc*& a,int sl )
{
	...
	for (int i = 0; i < sl; i++)
	{
		filein << i+1 << ',' << a[i].ten << ',' << a[i].khoa << ',' << a[i].so_luong << ',' << a[i].k << endl;
	}
	filein.close();
	delete[]address;
}
...
void xuat_lop_hoc(nam_hoc b,lop_hoc* a,int sl, int  x, int y)
{
	int py = y;
	x = 17;
	box(x, y, sl + 2, 80);
	...
	while (i < sl)
	{
			ToMau(x+2, y, a[i].ten, 7);
			ToMau(x+18, y, a[i].khoa, 7);
			gotoxy(x+42, y);
			textcolor(7);
			ds_sinh_vien l;
			init_ds_sinh_vien(l);
			doc_ds_sinh_vien_lop_hoc(b, a[i], l);
			cout << so_node_ds_sinh_vien(l)<<"/" << a[i].so_luong;
			ToMau(x+57, y, a[i].k, 7);
			y++;
			i++;
	}
	gotoxy(x-3, py+sl+2);
...
void xuat_ds_khoa_hoc(int x, int y, ds_khoa_hoc l,nam_hoc a , hoc_ki b)
{
	int  py = y;
	x = 12;
	system("cls");
	box(x, y, so_node_khoa_hoc(l)+2, 110);
	gotoxy(x, y);
	textcolor(2);
	...
	while (l.head != NULL)
	{
		...
		doc_ds_sinh_vien_khoa_hoc(a,b, l.head->data, ds_sv);
		cout <<so_node_ds_sinh_vien(ds_sv)<<"/"<< l.head->data.so_luong;
		gotoxy(x + 69, y);
...
bool check_ds_trong_lop(tai_khoan infor, nam_hoc a, hoc_ki b, khoa_hoc c)
{
	...
	{
		if (strcmp(ds_sv.head->data.ma_sinh_vien, infor.ms) == 0 && strcmp(ds_sv.head->data.cmnd, infor.cccd) == 0 && strcmp(ds_sv.head->data.ho_ten, infor.ho_ten)==0)
			return true;
...
void xem_khoa_hoc_da_dk(nam_hoc a, hoc_ki b, ds_khoa_hoc& l, int x, int y, tai_khoan t)
{
	...
	while (temp != NULL)
	{
		if (check_ds_trong_lop(t, a, b, temp->data) == true)
		{
			node_khoa_hoc* add = tao_node_mh(temp->data);
			addlast_mh(l, add);
		}
		temp = temp->next;
	}
...
int chon_sv_lop_hoc(ds_sinh_vien l, int x, int y)
{
	...
	while (true)
	{
		xuat_ds_sinh_vien_lop_hoc(l, x, y, pos);
		char c = _getch();
		if (c == 80)
		{
			if (pos == so_node_ds_sinh_vien(l))
				pos = 1;
			else pos++;
		}
		else if (c == 72)
